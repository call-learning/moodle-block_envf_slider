 ...
 * @category  files
 */
function block_envf_slider_pluginfile(
    $course,
    $birecordorcm,
    $context,
    $filearea,
    ...
    if ((!$file = $fs->get_file($context->id, 'block_envf_slider', $filearea, $itemid, $filepath, $filename)) ||
        $file->is_directory()) {
        send_file_not_found();
    }
 ...
 * @return void
 */
function block_envf_slider_global_db_replace($search, $replace) {
    global $DB;
 ...
 */
function block_envf_slider_get_path_from_pluginfile(string $filearea, array $args): array {
